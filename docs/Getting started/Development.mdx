...
The input command will display a list of openapi documents, identified by the `apiID`. Select the corresponding `apiID`. The docuo document will be generated internally according to the corresponding `apiID` configuration.
...
[OpenAPI configuration](../Using%20OpenAPI/OpenAPI%20Setup.mdx) is as follows:
...
The input command will display a list of openapi documents, identified by the `apiID`. Select the specified `apiID` to clear.
