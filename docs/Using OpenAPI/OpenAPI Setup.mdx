...
1. Define an `apiID` to uniquely identify the OpenAPI document. The value must be a string of lowercase letters and digits
2. Then add the paths to the OpenAPI document to the `specPath` field of the openapi config in your `docuo.config.json`:
...
To add the OpenAPI endpoints into your docs, each endpoint requires a corresponding MDX file. You can autogenerate these MDX files from your OpenAPI document using our `CLI`.

According to the following two parts:

1. First you need to add the paths to the OpenAPI document to the `outputDir` field of the openapi config in your `docuo.config.json`

<Tip>
It must correspond to the apiID of the OpenAPI document above.
</Tip>
